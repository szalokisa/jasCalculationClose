...
export default class Customerportal {
    ...
    async deleteById(props) {
        ...
        return await axios.delete(`${process.env.CUSTOMERPORTAL_BACKEND}/data`,
            {
                headers: {
                    'x-token': this.token,
                    'x-collection': props.collection,
                    'x-id': props.id,
                }
            })
    }

    async deleteByPartnerId(partnerId) {
        await this.tokenGet();
        return await axios.delete(`${process.env.CUSTOMERPORTAL_BACKEND}/users/partnerid`,
            {
                headers: {
                    'x-token': this.token,
